(function(){
  ...
  function saveHistory(){
    ...
    if(unique.length>MAX_HISTORY) history=unique.slice(unique.length-MAX_HISTORY);
    localStorage.setItem(HISTORY_KEY,JSON.stringify(history));
  }
  function addHistory(ids){
    ids.forEach(id=>{if(Number.isInteger(id)&&!history.includes(id))history.push(id)});
    saveHistory();
  }
  ...
  function buttonFor(ti,pi){
    const cards=document.querySelectorAll('#teams .team-card');
    const card=cards[ti];
    return card?.querySelectorAll('.reroll-one')[pi]||null;
  }
  function currentIds(){return snapshot().flat()}
  function markVarietyStatus(){
    const status=document.querySelector('#status');
    if(!status)return;
    const base=status.textContent||'Pokédex lista';
    if(!/variedad/i.test(base))status.textContent=`${base} · 🎲 ${history.length} descartados`;
  }
  async function resolveRepeatedSlots(){
    ...
    now.forEach((team,ti)=>{
      (team||[]).forEach((p,pi)=>{
        const id=p?.id;
        const old=previous[ti]?.[pi];
        if(Number.isInteger(id)&&id!==old)changed.push({ti,pi,id});
      });
    });
    ...
    try{
      for(const slot of changed){
        let attempts=0;
        let latest=teams();
        let id=latest[slot.ti]?.[slot.pi]?.id;
        while(Number.isInteger(id)&&history.includes(id)&&attempts<25){
          const btn=buttonFor(slot.ti,slot.pi);
          if(!btn)break;
          attempts++;
          btn.click();
          await new Promise(r=>setTimeout(r,45));
          latest=teams();
          id=latest[slot.ti]?.[slot.pi]?.id;
        }
      }
      previous=snapshot();
      localStorage.setItem(SNAP_KEY,JSON.stringify(previous));
  ...
  function init(){
    ...
    if(!history.length&&initial.length) addHistory(currentIds());
    previous=initial;
    localStorage.setItem(SNAP_KEY,JSON.stringify(previous));
    markVarietyStatus();
    setInterval(()=>{
      const now=snapshot();
      if(!same(now,previous)) resolveRepeatedSlots();
    },250);
  }
