(function(){
  ...
  function wheelText(o){
    if(o.format==='3v1')return `${o.teamA.map(i=>`J${i+1}`).join(' + ')}\\nVS\\n${o.teamB.map(i=>`J${i+1}`).join(' + ')}`;
    if(o.format==='2v2')return `${o.teamA.map(i=>`J${i+1}`).join(' + ')}\\nVS\\n${o.teamB.map(i=>`J${i+1}`).join(' + ')}`;
  ...
  window.renderRoulette=function(){
    ...
    opts.forEach((o,i)=>{
      const m=document.createElement('div');m.className='wheel-marker';
      const a=(-90+span*(i+.5))*Math.PI/180;
      ...
      const item=document.createElement('div');item.className='roulette-option'+(currentMatchup?.optionIndex===i?' is-winner':'');
      const t=matchupText(o);item.innerHTML=`<div class="option-main">${t.main}</div><div class="option-sub">${t.sub}</div>`;rouletteOptionsEl.appendChild(item);
    });
  };

  window.renderVersus=function(reveal=false){
    versusEl.classList.remove('reveal');
    if(!currentMatchup||currentMatchup.players!==+$('#players').value){
      versusEl.innerHTML='<div class="versus-placeholder">Gira la ruleta para revelar la combinación.</div>';return;
    }
    currentMatchup=ensureBattleMeta(currentMatchup);localStorage.setItem(MATCHUP_KEY,JSON.stringify(currentMatchup));
    const a=currentMatchup.featured?.a,b=currentMatchup.featured?.b;
  ...
  window.randomizeVersus=function(){
    if(spinning)return;
    spinning=true;ensureAudio();
    const btn=$('#versusBtn'),opts=matchupOptions(),winner=Math.floor(Math.random()*opts.length);
    const next=ensureBattleMeta({...opts[winner],optionIndex:winner});
    ...
    versusEl.innerHTML='<div class="versus-placeholder">Sorteando enfrentamiento…</div>';
    wheelEl.style.transform=`rotateZ(${wheelRotation}deg)`;
    const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
    let ticks=reduced?null:setInterval(()=>tone(1180,.025,.014),105);
