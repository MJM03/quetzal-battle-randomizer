(function(){
  ...
  function wheelText(o){return o.format==='3v1'?'3V1':o.format==='2v2'?'2V2':'1V1'}
  ...
  function safeArena(teamA,teamB,a,b,seeds){
    try{
      const sa=sideMarkup('a',teamA,a,seeds?.[0]);
      const sb=sideMarkup('b',teamB,b,seeds?.[1]);
      return `<div class="battle-arena">${sa}<div class="battle-vs-core">VS</div>${sb}</div>`;
    }catch(err){
      console.warn('QBR: no se pudo renderizar la arena gráfica',err);
      return `<div class="battle-arena compact-arena"><div class="compact-feature"><strong>${a?displayName(a.player):'Equipo A'}</strong><span>${a?`${a.name||'Pokémon'} · ${dex(a.id)}`:'Sin Pokémon destacado'}</span></div><div class="battle-vs-core compact">VS</div><div class="compact-feature"><strong>${b?displayName(b.player):'Equipo B'}</strong><span>${b?`${b.name||'Pokémon'} · ${dex(b.id)}`:'Sin Pokémon destacado'}</span></div></div>`;
    }
  }
  window.renderVersus=function(reveal=false){
    try{
      ...
      /* Primero construimos TODO el resultado en memoria. Así un error de sprites nunca impide mostrar los equipos. */
      ...
      const html=`<div class="battle-card${reveal?' reveal':''}">
      ...
      versusEl.innerHTML=html;
    }catch(err){
      console.error('QBR: error mostrando enfrentamiento',err);
      /* Fallback final: el usuario siempre debe ver qué equipos salieron. */
      if(currentMatchup&&Array.isArray(currentMatchup.teamA)&&Array.isArray(currentMatchup.teamB)){
        versusEl.innerHTML=`<div class="battle-card reveal"><div class="battle-card-head"><span class="battle-card-title">ENFRENTAMIENTO</span></div><div class="formed-teams-grid">${formedTeamBlock('EQUIPO A',currentMatchup.teamA,'a')}<div class="formed-vs">VS</div>${formedTeamBlock('EQUIPO B',currentMatchup.teamB,'b')}</div></div>`;
  ...
  window.randomizeVersus=function(){
    ...
    wheelRotation+=5*360+(target-current+360)%360;btn.disabled=true;
    versusEl.innerHTML='<div class="versus-placeholder">Sorteando enfrentamiento…</div>';wheelEl.style.transform=`rotateZ(${wheelRotation}deg)`;
    ...
    setTimeout(()=>{
      ...
      try{
        currentMatchup=ensureBattleMeta(next);
        localStorage.setItem(MATCHUP_KEY,JSON.stringify(currentMatchup));
      }catch(err){
        console.warn('QBR: meta del enfrentamiento falló, usando combinación base',err);
        currentMatchup=next;
        localStorage.setItem(MATCHUP_KEY,JSON.stringify(currentMatchup));
      }
      spinning=false;btn.disabled=false;renderRoulette();renderVersus(true);
      try{successSound()}catch{}
      try{confetti()}catch{}
      setTimeout(()=>versusEl.scrollIntoView({behavior:'smooth',block:'nearest'}),80);
    },reduced?80:3650);
  };
