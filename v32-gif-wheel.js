...
(function(){
  ...
  function getName(img){
    ...
    return slug(wrap?.dataset?.pokemonName||img.dataset?.pokemonName||img.alt||'');
  }
  function getId(img){
    const wrap=img.closest('.team-sprite-wrap');
    return Number(wrap?.dataset?.pokemonId||img.dataset?.pokemonId||0);
  }
  function applyTo(img){
    ...
    const name=getName(img),id=getId(img);
    if(!name)return;
    ...
    img.onerror=null;
    img.src=gif;
    img.onerror=function(){img.onerror=null;if(id)img.src=HOME_BASE+id+'.png'};
  }
