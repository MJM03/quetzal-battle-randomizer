/* V36 — GIFs animados en ruleta, tarjetas y enfrentamiento */
(function(){
  ...
  const SELECTOR='#teamSpriteLayer img.team-sprite,.pokemon-card img,.featured-pokemon,.battle-arena img,.fighter-pokemon';
  function slug(raw){
    let s=String(raw||'').toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    s=s.replace(/♀/g,'-f').replace(/♂/g,'-m').replace(/[.'’]/g,'').replace(/\s+/g,'-');
    s=s.replace(/^mega-(.+)-(x|y)$/,'$1-mega-$2').replace(/^mega-(.+)$/,'$1-mega');
    s=s.replace(/^primal-(.+)$/,'$1-primal');
    return s;
  }
  ...
  function nameOf(img){
    ...
    return wrap?.dataset?.pokemonName||img.dataset.pokemonName||img.alt||img.closest('[data-pokemon-name]')?.dataset?.pokemonName||'';
  }
  function arm(img){
    ...
    img.dataset.qbrGifFallback=String(id||0);
    img.onerror=null;
    img.src=gif;
    img.onerror=function(){
      img.onerror=null;
      const n=Number(img.dataset.qbrGifFallback||0);
      if(n)img.src=HOME+n+'.png';
    };
  }
    ...
    const obs=new MutationObserver(muts=>{
      for(const m of muts){
        ...
        if(m.type==='attributes'&&m.target instanceof HTMLImageElement&&m.attributeName==='src'){
          const img=m.target;
          if(img.dataset.qbrGif && (img.currentSrc||img.src).split('?')[0]!==img.dataset.qbrGif)arm(img);
        }
      }
    });
